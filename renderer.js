...
const defaultConfigValues = {
    ...
    qsRouterContract: '0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff',
    wmaticContract: '0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270',
    qsFactoryContract: '0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32',
    rpcURL: [
    ...
    if (amount > 0)
        return `<div class="col s3">
                <div class="chip green white-text">
                    ${status === 'bought' ? 'Comprado' : 'Vendido'}: ${amount} MATIC
                </div>
            </div>`
    return ''
}
...
function getTokenHTML(token) {
    return `<li id="${token.address}">
                ...
                <span class="badge green">Liquidez: ${token.liquidity.toFixed(2)} MATIC</span>
                </div>
                ...
                <div class="row">
                    ...
                    ${getBuyButton(token)}
                    <div class="col s3">
                        <button onclick="openInBrowser('${getNetworkExplorer(token.network, 'token')}${token.address}')" class="btn blue-grey">Ver en polygonscan</button>
                    </div>
                    <div class="col s3">
...
function getProfitsHTML(token) {
    return token.status === 'bought' ? `<div class="col s2">
                <span class="badge ${token.profitPercent >= 0 ? 'green' : 'red'}">Actual: ${getCurrentValue(token.profit, token.buyAmount)} MATIC</span>
            </div>
            <div class="col s3">
                <span class="badge ${token.profitPercent >= 0 ? 'green' : 'red'}">Profit: ${token.profit} MATIC</span>
            </div>
            <div class="col s3">
...
function getAppConfig() {
    ...
    const qsRouterContract = document.getElementById('qs-router_contract')
    const wmaticContract = document.getElementById('wmatic_contract')
    const qsFactoryContract = document.getElementById('qs-factory_contract')
    ...
    userConfig.qsRouterContract = qsRouterContract.value || qsRouterContract.placeholder || userConfig.qsRouterContract
    ...
    userConfig.wmaticContract = wmaticContract.value || wmaticContract.placeholder || userConfig.wmaticContract
    userConfig.qsFactoryContract = qsFactoryContract.value || qsFactoryContract.placeholder || userConfig.qsFactoryContract
...
function setAppConfig() {
    ...
    document.getElementById('qs-router_contract').placeholder = userConfig.qsRouterContract
    document.getElementById('wmatic_contract').placeholder = userConfig.wmaticContract
    document.getElementById('qs-factory_contract').placeholder = userConfig.qsFactoryContract
