...
const getPairAddress = async (config, tokenAddress) => {
    const factory = (await Contract.Instance(types.FACTORY, config.qsFactoryContract, config)).methods
    return await factory.getPair(tokenAddress, config.wmaticContract).call()
}
