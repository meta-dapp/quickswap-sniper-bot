...
const _getEvents = async (factory, fromBlock, config) => {
    ...
    if (!config.autoSniping)
        options['filter'] = { token0: config.tokenAddress, token1: config.wmaticContract }
...
const subscribePairCreated = async (contract, config, callback) => {
    ...
    while (!stopPairSubscription) {
        try {
            ...
            const factory = await contract.Instance(Types.FACTORY, config.qsFactoryContract, config)
            const wmatic = await contract.Instance(Types.TOKEN, config.wmaticContract, config)
            ...
            if (events && events.length > 0) {
                for (var i = 0; i < events.length; i++)
                    if (events[i].event === 'PairCreated') {
                        ...
                        const maticBalance = await wmatic.methods.balanceOf(pairData.pair).call()

                        pairData['liquidity'] = {
                            matic: FromWei(maticBalance, 18),
                            good_liquidity: FromWei(maticBalance, 18) > config.minLiquidity
                        }

                        if (!isTokenCached(token0.toLowerCase() === config.wmaticContract.toLowerCase() ? token1.toLowerCase() : token0.toLowerCase())) {
