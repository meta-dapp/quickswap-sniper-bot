    ...
    Web3Wss.subscribePairCreated(Contract, config, async (pairs) => {
        for (var i = 0; i < pairs.length; i++) {
            ...
            if (pair.liquidity.good_liquidity) {
                if (pair.token0.toLowerCase() === config.wmaticContract.toLowerCase()
                    || pair.token1.toLowerCase() === config.wmaticContract.toLowerCase()) {
                    ...
                    pair['liquidity'] = pair.liquidity.matic
...
const Sell = async (token, _store) => {
    ...
    const ContractPCS = (await Contract.Instance(Types.ROUTER, config.qsRouterContract, config)).methods
    const ContractToken = (await Contract.Instance(Types.TOKEN, token.address, config)).methods
    const tokenIn = token.address
    const tokenOut = config.wmaticContract
    ...
    const allowance = await ContractToken.allowance(config.userAddress, config.qsRouterContract).call()
    ...
    if (tx.status) {
        ...
        var amountOutMin = Decode.FromWei(amounts[amounts.length - 1], config.wmaticContract)
        ...
        log(chalk.red(`
         ...
         amountOut: ${parseFloat(expectedAmount)} ${tokenOut} WMATIC
       `))
...
const Buy = async (token, _store) => {
    ...
    const ContractPCS = (await Contract.Instance(Types.ROUTER, config.qsRouterContract, config)).methods
    const ContractWMATIC = (await Contract.Instance(Types.TOKEN, config.wmaticContract, config)).methods
    const tokenIn = config.wmaticContract
    const tokenOut = token.address

    const amountIn = Decode.ToWei(amount, 18)

    log(chalk.yellow('Checking WMATIC allowance...'))
    var tx = {}
    const supply = await ContractWMATIC.totalSupply().call()
    const allowance = await ContractWMATIC.allowance(config.userAddress, config.qsRouterContract).call()
    ...
    if (Decode.FromWei(allowance.toString(), token.decimals) >= Decode.FromWei(supply.toString(), token.decimals)) {
        ...
        log(chalk.yellow('Approving...'))
        tx = await ContractWMATIC.approve(
            config.qsRouterContract,
            Web3.utils.toBN('0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff').toString()
        ).send()
    }
    ...
    if (tx.status) {
        ...
        log(chalk.green(`
         Buying ${tokenOut}
         =================
         amountIn: ${amount.toString()} ${tokenIn} WMATIC
